...
public class BlockStateSaverAndLoader extends PersistentState {
    ...
    public static BlockStateSaverAndLoader createFromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup lookup) {
        ...
        for (int i = 0; i < blockEnchantmentsList.size(); i++) {
            ...
            // 将读取的方块附魔信息添加到列表中
            blockEnchantments.add(new BlockEnchantInfo(blockPos, enchantments));
        }

        return state;
    }

    private static final Type<BlockStateSaverAndLoader> type = new Type<>(
    ...
    public static BlockStateSaverAndLoader getServerState(MinecraftServer server) {
        ...
        if(server!=null) {
            ...
            BlockStateSaverAndLoader state = persistentStateManager.getOrCreate(type, Thinking.MOD_ID + "_block_enchantments");
